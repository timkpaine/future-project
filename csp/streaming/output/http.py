...
from ...base import StreamEnd, CSPException
...
class HTTPServer(Func):
    ...
    def __init__(
        ...
        if meth.lower() not in ("get", "post", "both"):
            raise CSPException("`meth` must be in ('get', 'post', 'both')")

        # instantiate server if not existing
        server = server or web.Application()

        # create asyncio queue to manage between the http server and the csp func
        queue = asyncio.Queue()
